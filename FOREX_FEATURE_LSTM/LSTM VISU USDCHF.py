# -*- coding: utf-8 -*-
"""
Created on Mon Jul  6 01:07:58 2020
...
df = pd.read_csv('C:/Users/yiming/Feature Eng/USDCHF_FULL_DATA.csv')
df.Date = pd.to_datetime(df.Date)
df = df.set_index('Date')
train,test = df['USDCHF=X'][:53],df['USDCHF=X'][53:]
...
# define input sequence
in_seq3 = df['Vol_local'][:53].values
in_seq7 = df['FD'][:53].values
in_seq8 = df['Energy'][:53].values
in_seq14 = df['Distance_MA'][:53].values
out_seq = train.values
...
out_seq = out_seq.reshape((len(out_seq), 1))
# horizontally stack columns
dataset = hstack((in_seq3,in_seq7,in_seq8,in_seq14,out_seq))
# choose a number of time steps
n_steps = 3
# convert into input/output
X, y = split_sequences(dataset, n_steps)
